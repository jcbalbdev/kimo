...
Deno.serve(async () => {
  ...
  const { data: birthdays } = await supabase.rpc('get_pet_birthdays_today');

  let sent = 0;

  for (const row of birthdays ?? []) {
    const title = `🎂 ¡Feliz cumpleaños, ${row.pet_name}!`;

    // Personalize by age
    const body = row.years_old === 1
      ? `${row.pet_name} cumple su primer añito hoy 🐾 — dale mucho amor`
      : `${row.pet_name} cumple ${row.years_old} años hoy 🎉 — ¡celébralo con KIMO!`;
